...
**Important 2026 API change:**
In early 2026, the library removed its static methods. The old pattern was:
```python
# OLD (broken in 2026):
YouTubeTranscriptApi.get_transcript(video_id)
```
The new pattern uses instance-based calls:
```python
# NEW (correct in 2026):
...
```python
    ...
    try:
        ...
        pass
```
- Tries to check if captions exist. If this fails for any reason, we silently set `has_captions = False` — it's not critical for the validate endpoint.

```python
    metadata["has_captions"] = has_captions
    return metadata
```
- Adds the `has_captions` field to the metadata dict and returns it.
...
```
- `task_result.state` — One of: `PENDING`, `STARTED`, `FETCHING_METADATA`, `FETCHING_CAPTIONS`, `DOWNLOADING_AUDIO`, `CONVERTING_AUDIO`, `UPLOADING`, `HANDOFF`, `SUCCESS`, `FAILURE`.
- `task_result.info` — The `meta` dictionary from `self.update_state(...)` in the task.
- `task_result.result` — The final return value of the task (only available when state is `SUCCESS`).
...
def fetch_captions(video_id: str) -> Optional[str]:
    try:
        ytt_api = YouTubeTranscriptApi()
```
- Creates a new instance of the transcript API. **Must be instantiated** — the old static method pattern (`YouTubeTranscriptApi.get_transcript(...)`) was removed in the 2026 update of the library.
...
```
- **First try:** Get English captions (`languages=["en"]`).
- **If no English captions exist:** Falls back to getting a list of all available transcripts and taking the first one (could be Hindi, Spanish, auto-generated, etc.).
...
```python
        ...
        return None
```
- The transcript comes as a list of dictionaries: `[{"text": "Hello", "start": 0.0, "duration": 1.5}, ...]`.
- We join all the `text` fields into a single string.
...
```python
        ...
        return None
```
- **Silent failure.** If anything goes wrong (network error, unknown exception), we return `None`. This tells the caller "no captions available", which triggers the audio download fallback. We do NOT crash the pipeline.
...
```
- Uploads the file with the key `audio/FMOtyYU6pj8.wav` (like a file path inside the bucket).
...
The task progresses through these states in order:

```mermaid
flowchart TD
    A["FETCHING_METADATA\n(Get title, duration, etc.)"] --> B["FETCHING_CAPTIONS\n(Try to get subtitles)"]
    B --> C{Captions found?}
    C -->|Yes| G["HANDOFF\n(Pass data to Module 4)"]
...
Here's what happens from the moment a user submits a URL to the moment the result is ready:
...
sequenceDiagram
    ...
    W->>YT: youtube-transcript-api<br/>get captions
...
- **Input:** `{"url": "https://youtu.be/FMOtyYU6pj8?si=Y7SltqTRLlz_8Anf"}`
- **Output:**
  ```json
  {
    "title": "How to get Placement in 6 months ? Study Plan for Placements | Software Engineers",
    "channel_name": "Apna College",
    "duration_seconds": 1690,
    "thumbnail_url": "https://i.ytimg.com/vi/FMOtyYU6pj8/hqdefault.jpg",
    "has_captions": false
  }
  ```
- **What happened:** URL validated → Video ID extracted → YouTube API called → Metadata returned → Captions checked (none found) → Response sent.

### Test 2: `/process` endpoint
- **Input:** Same URL.
- **Output:** `{"task_id": "b8e9276e-efa4-4c70-8dd5-c9a67370d6ce"}`
- **What happened in the Celery worker terminal:**
  ```
  [download] 100% of 26.08MiB in 00:00:04 at 5.83MiB/s
  Handoff data for M4: {'video_id': 'FMOtyYU6pj8', 'metadata': {...},
    'has_captions': False,
    's3_audio_uri': 's3://video-synopsis-audio/audio/FMOtyYU6pj8.wav',
    'captions_extracted': False}
  Task succeeded in 9.95s
  ```

### Test 3: `/status/{task_id}` endpoint
- **Input:** `b8e9276e-efa4-4c70-8dd5-c9a67370d6ce`
- **Output:** `{"task_id": "b8e9276e-...", "state": "SUCCESS", "progress": null, "result": null}`

### Test 4: MinIO Console
- Opened `http://localhost:9001`, logged in with `minioadmin/minioadmin`.
- Navigated to Object Browser → `video-synopsis-audio` → `audio/`.
- Confirmed `FMOtyYU6pj8.wav` was present.
...
In Phase A, our task just logs the handoff data:
```python
logger.info(f"Handoff data for M4: {result}")
```

In Phase B, this becomes a real function call:
```python
...
- The S3 URI of the audio file → so M4 can download it.
